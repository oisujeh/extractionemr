package org.openmrs.module.extractionemr.ndrUtils;
...
import org.openmrs.module.extractionemr.model.ndr.FacilityType;
...
import org.openmrs.module.extractionemr.ndrfactory.*;
...
import org.openmrs.module.extractionemr.util.ZipUtil;
...
public class Utils {
	...
	private static final ExtractionObsService extractionObsService = Context.getService(ExtractionObsService.class);
	...
	public static String getIPReportingState() {
		...
		if(stateCode == null || "UNKNOWN".equalsIgnoreCase(stateCode) || stateCode.isEmpty() ||
			...
			Optional<DatimMap> datimMap = Optional.ofNullable(extractionemrService.getDatatimMapByDataimId(datimCode));
	...
	public static List<Obs> extractObsList(List<Integer> allPatientObsList, Integer[] encounterTypeArr) {
		...
		return extractionObsService.getObsByEncounterTypes(allPatientObsList, encounterTypeList, false);
	}
	...
	public static List<Obs> extractObsPerVisitDate(Date visitDate, List<Integer> obsList) {
		return extractionObsService.getObsByVisitDate(visitDate, obsList, false);
	}
	
	public static Obs extractObs(int conceptID, List<Integer> obsList) {
		Concept concept = Context.getConceptService().getConcept(conceptID);
		return extractionObsService.getLastObsByConcept(concept, obsList);
	}
	...
	public static List<Obs> extractObsList(int conceptID, List<Integer> obsList) {
		Concept concept = Context.getConceptService().getConcept(conceptID);
		return extractionObsService.getObsByConcept(concept, obsList);
	}
	...
	public static List<Integer> getCareCardObs(Patient patient, Date endDate) {
		ExtractionEncounterService extractionEncounterService = Context.getService(ExtractionEncounterService.class);
		Encounter hivEnrollmentEncounter = extractionEncounterService.getLastEncounterByEncounterTypeIds(patient,null, endDate,Arrays.asList(Care_card_Encounter_Type_Id));
		if (hivEnrollmentEncounter != null) {
	...
	public static List<Encounter> getAllRegimenObs(Patient patient) {
		ExtractionEncounterService extractionEncounterService = Context.getService(ExtractionEncounterService.class);
		return extractionEncounterService.getEncountersByEncounterTypeIds(patient, null, null,
	...
	public static Map<Integer, Obs> groupObsByConceptId(List<Integer> obsCodeList, List<Integer> obsPerVisitDateIds) {
		...
		List<Concept> concepts = extractionObsService.getConcepts(obsCodeList, false);
		List<Obs> obsList = extractionObsService.getObsByConcept(concepts, obsPerVisitDateIds);
		if(obsList != null){
			for(Obs obs : obsList){
	...
	public static List<Encounter> getLastEncounters(Patient patient, Date endDate) {
		ExtractionEncounterService extractionEncounterService = Context.getService(ExtractionEncounterService.class);
		return extractionEncounterService.getEncountersByEncounterTypeIds(patient, null, endDate,
	...
	public static List<Obs> getObs(Patient patient, int conceptId) {
		List<Obs> obsList = extractionObsService.getObsByConceptId(patient.getPersonId(), conceptId, null, false);
		if (obsList != null) {
			return obsList;
		}
		return null;
	...
	public static List<Obs> getObs(Patient patient, int conceptId, int encounterId) {
		return extractionObsService.getObsByConceptId(patient.getPersonId(), conceptId, Arrays.asList(encounterId), false);
	}
