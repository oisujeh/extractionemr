...
<div class="row wrapper  white-bg page-heading"  style="">
    ...
    <h4 style="text-align: center">
       Recaptured Fingerprint NDR Export
    ...
    <div id="gen-wait" class="dialog" style="display: none; ">
        <div class="row">
            <div class="col-md-3 col-xs-3 offset-2" >
                <img src="../moduleResources/extractionemr/images/Sa7X.gif" alt="Loading Gif"  style="width:100px">
            </div>
        </div>
...
<script>
    ...
    {
        const checkBox = document.getElementById("custom");
        let url = "${ ui.actionLink("extractionemr", "ndr", "generateNDRFile") }"
        if (checkBox.checked === true){
            url = "${ ui.actionLink("extractionemr", "ndr", "generateCustomNDRFile") }"
        }
    ...
    function loadFileListDefault(showProgressDialog) {
        ...
        const checkBox = document.getElementById("custom");
        let url = "${ ui.actionLink("extractionemr", "ndr", "getFileList") }"
        if (checkBox.checked === true){
            url = "${ ui.actionLink("extractionemr", "ndr", "getManualFileList") }"
        }
                ...
                if(fileListObj[i].active) {
                    ...
                    jq('#TableBody')
                        .append("<tr style=\"opacity:0.6;filter: alpha(opacity = 60)\">" +
                            ...
                            "<td>" + fileListObj[i].status + "</td>" +
                            "<td>" +
                            "<img id=\"loadingImg"+i+"\" src=\"../moduleResources/extractionemr/images/Sa7X.gif\" alt=\"Loading Gif\"  style=\"width:25px\"> <p>"+fileListObj[i].progress+"</p>" +
    ...
    function restartErrorFile(id){
        if (confirm("Are you sure you want to restart ? this will rerun failed files?") === true) {
            ...
            if(id)
            {
                console.log(id);
                jq.ajax({
                    url: "${ ui.actionLink("extractionemr", "ndr", "restartFile") }",
                    dataType: "json",
                    data: {
                        'id' : id,
                        'action' : 'failed'
    ...
    function resumeFile(id){
        if (confirm("Are you sure you want to resume ?") === true) {
            ...
            if(id)
            {
                console.log(id);
                jq.ajax({
                    url: "${ ui.actionLink("extractionemr", "ndr", "resumeFile") }",
                    dataType: "json",
                    data: {
                        'id' : id
                    }
    ...
    function deleteFile(id){
        if (confirm("Are you sure you want to delete this file ?") === true) {
            ...
            if(id)
            {
                console.log(id);
                jq.ajax({
                    url: "${ ui.actionLink("extractionemr", "ndr", "deleteFile") }",
                    dataType: "json",
                    data: {
                        'id' : id
                    }
    ...
    function pauseFile(id){
        if (confirm("Are you sure you want to pause the process ?") === true) {
            ...
            if(id)
            {
                console.log(id);
                jq.ajax({
                    url: "${ ui.actionLink("extractionemr", "ndr", "pauseFile") }",
                    dataType: "json",
                    data: {
                        'id' : id
                    }
